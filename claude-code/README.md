...
- **Hooks** (`hooks/hooks.json`): `PreToolUse` guard that blocks scans against non-localhost targets
...
The MCP server and the Repeater both read `BRIGHT_HOSTNAME` and `BRIGHT_TOKEN`. Scans only
target `localhost` in the current execution environment — the hook enforces this.
